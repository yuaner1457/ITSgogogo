...
# 生成所有航迹对的索引
pairs = [(i, j) for i in range(num_tracks) for j in range(i + 1, num_tracks)]
# 计算DTW距离
def cal_dtw(pair):
    i,j=pair
    return i, j, DTW(tracks[i], tracks[j])
import multiprocessing
if __name__ == '__main__':
    # 使用 freeze_support()，如果要生成可执行文件则需要
    multiprocessing.freeze_support()
    # 执行多进程任务
    with multiprocessing.Pool() as pool:
        results = pool.map(cal_dtw, pairs)  # 例如对DTW计算进行并行处理
    # 初始化DTW距离矩阵
    ...
    for i, j, distance in results:
        ...
        dtw_matrix[j, i] = distance  # 确保矩阵是对称的
    ...
    np.savetxt("dtw_2000.csv", dtw_matrix, delimiter=',')
